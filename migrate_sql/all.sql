-- 特規版精簡：已移除功能（TwitCasting、CHZZK、YouTube 會員驗證、Twitch 訂閱驗證、
-- 錄影委派、Holo/Nijisanji 分類、Google OAuth unlink、Twitter Space）不再建立對應資料表與欄位；
-- 對應 migration 僅保留 __EFMigrationsHistory 記錄，避免 EF 判定未套用而重建已移除的物件。
