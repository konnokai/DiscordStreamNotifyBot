...
- 進行中：**三層拆分重構（Scraper / Notifier / Coordinator + Shared，Redis Streams 匯流排）**，權威設計與分階段步驟見 [docs/HORIZONTAL_SCALING_PLAN.md](docs/HORIZONTAL_SCALING_PLAN.md)，目前進度：**階段 6 檔案完成（Dockerfile + compose 方式 A + .env.example + .dockerignore），階段 7（制度回填）待做**。§11 驗證清單與 Docker 實跑需測試環境（本機無 docker）。正確性（§11）待計畫完成後於測試環境驗。
