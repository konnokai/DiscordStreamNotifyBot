...
description: >
  ...
  涵蓋 Shared（DTO / NotifyType / Redis 頻道 / 無狀態 API）→ Scraper/Detection（偵測 → publish DTO 到 Redis Streams）
  ...
  「事件要過匯流排」，就用這個 skill，即使他沒講到「偵測服務」或「匯流排」這些字。
---

# 新增偵測平台 / 通知事件

這個專案把「偵測」與「發送」拆到不同程序，靠 **Redis Streams 匯流排**（`bot:notify`）解耦。理由：偵測者只能有一個
...
- `Shared/Messages/Notifications.cs`：
  ...
  - 若事件有多種類型，新增 `XxxNoticeType` 線路列舉（與 UI 用的 `NoticeType` 分開，只當跨層契約，成員順序須對應）。
  - 在 `NotifyType`（static class of const string）加一個型別常數，例如 `public const string Kick = "kick";`。
...
- 建 `Scraper/Detection/Xxx/XxxDetectionService.cs`：
  - **只 using Shared**，不得參考 Discord.WebSocket / Notifier。用 `using Bot = BotState;` 別名取 `Bot.RedisDb`/`Bot.RedisSub`。
  ...
  - 偵測到事件 → `await NotificationBus.PublishAsync(Bot.RedisDb, NotifyType.Xxx, dto)`
    （XADD 至 `bot:notify`，自帶 `MAXLEN ~` 修剪）。
  - 錄影等副作用（Redis publish 給錄影工具）在偵測端完成，結果用 DTO 欄位（如 `IsRecord`）帶過去。
...
- 建 `Notifier/SharedService/Xxx/XxxService.cs : IInteractionService`：
  ...
  - 通知設定清單走記憶體快取（參考 `Notifier/SharedService/NoticeCache.cs`，TTL + 變更 Invalidate），不要每次打 DB。
...
- `Notifier/NotificationBusConsumer.cs`：**這一步要手動**——建構子注入新的 `XxxService`，在 `DispatchAsync`
  的 switch 加 `case NotifyType.Xxx:` 反序列化 DTO 後呼叫 `DispatchFromBusAsync`。漏了這步，事件會被
  `尚未接線的 type: {type}，暫時 ack 略過` warn 後直接 ack 丟掉。

## 收尾檢查
- publisher 與 consumer 的型別字串一致（用 `NotifyType` 常數，別寫字面字串）。
- 偵測服務沒有任何 `using Discord*`；發送服務沒有任何 Timer/偵測邏輯。
- at-least-once：發送成功後寫去重鍵 `notified:...`（見 `NotificationBusConsumer.TryGetDedupKey`），重複訊息直接 ack 略過。
...
- 端到端驗證走 [debug-detection-bus](../debug-detection-bus/SKILL.md) 的路徑（需 Scraper + Redis + Notifier 同時跑）。
